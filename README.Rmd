...
The GIMME algorithm can be implemented in the R package, *gimme*.

Continually updated tutorials can be found in the [GIMME Online Tutorials](https://tarheels.live/gimme/tutorials/).

These papers also provide insights for researchers wishing to get started with the R version of *gimme*: [Automated Selection of Robust Individual-Level Structural Equation Models for Time Series Data](http://gimme.web.unc.edu/files/2014/12/Lane-Gates-2017-SEM-gimme.pdf) and [Network Mapping with GIMME](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6181449/pdf/nihms-1502668.pdf).

Program developers are invited to submit changes here at the GitHub repository. 

Please email here with bugs: [gimme@unc.edu](mail.to:gimme@unc.edu). 

Please post questions regarding interpretation, study design, and other topics here: [GIMME google group](https://groups.google.com/g/gimme-r)

# **The Basics**


+ GIMME can be used to estimate the unified SEM (uSEM; Kim et al., 2007; Gates et al., 2010), extended unified SEM (euSEM; Duffy et al., 2021; Gates et al., 2011); hybrid-uSEM (Luo et al., under review; Molenaar, 2019), and the standard VAR with correlated errors.

+ Missing data is not a problem. 

+ Heterogeneity in processes across individuals is not a problem: 
...
__1. Create your data: Extract the time series for your variables__
  ...
  + If storing the data in a directory, store each individual's data in a different file. Files must be either comma, space, or tab delimited. Ensure nothing else is in this directory except data files you wish to include in the analyses. 
  
  + If storing your data as a list, store each individual's data frame in a different slice of this list. 
  
__2. Installing gimme with R__
...
__3. Running gimme__
   ...
   The *gimme()* (or equivalently, *gimmeSEM()*) function contains these base options (see online tutorial for updates):
...
fit <- gimme(         # can use "gimme" or "gimmeSEM"
  data = '',          # source directory where your data are, or name of list
  out = '',           # optinal output directory where you'd like your output to go
  sep = "",           # how data files are separated. "" for space; "," for comma, "/t" for tab-delimited 
  header = ,          # TRUE or FALSE, is there a header (if reading in from directory)
...
5. If all of this is correct, please email the error you received, code used to run *gimme*, and the data (we promise not to use it or share it) to: gimme@unc.edu. 
