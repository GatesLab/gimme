...
gimmeSEM <- gimme <- function(data             = NULL,
  ...
  # create.tree(grp_history) # delete
  
  
  #-------------------------------------------------------------#
  # Create group object from grp_history
  #-------------------------------------------------------------#
  
  grp <- replicate(length(grp_history[[length(grp_history)]]), grp, simplify = FALSE)
  
  grp <- lapply(seq_along(grp), function(i){
    grp[[i]]$n_group_paths <- grp_history[[length(grp_history)]][[i]]$n_paths
    grp[[i]]$group_paths   <- grp_history[[length(grp_history)]][[i]]$add_syntax
    grp[[i]]
  })
   
  #-------------------------------------------------------------#
  # Update grp_history with stage and prune info.
  #-------------------------------------------------------------#
  
  # grp_history[[length(grp_history)]] <- lapply(seq_along(grp_history[[length(grp_history)]]), function(i){
  #     grp_history[[length(grp_history)]][[i]]$pruned <- NA
  #     grp_history[[length(grp_history)]][[i]]$stage  <- "group"
  #     grp_history[[length(grp_history)]][[i]]
  #     
  # })
  
  grp_history <- lapply(seq_along(grp_history), function(i){
     lapply(seq_along(grp_history[[i]]), function(j){
       grp_history[[i]][[j]]$pruned     <- NA
       grp_history[[i]][[j]]$stage      <- "group"
       grp_history[[i]][[j]]$grp_sol    <- NA
       grp_history[[i]][[j]]$subgrp_sol <- NA
       grp_history[[i]][[j]]
     })
  })
  ...
  if (any(prune)){
    ...
    grp_history[[length(grp_history)]] <- lapply(seq_along(grp_history[[length(grp_history)]]), function(i){
      ...
      if(length(pruned) != 0){
        grp_history[[length(grp_history)]][[i]]$pruned <- pruned
      }
      grp_history[[length(grp_history)]][[i]]
    })
    ...
    #-----------------------------------------------------------#
    # Update the grp object to reflect the pruning.
    #-----------------------------------------------------------#
  ...
  #-------------------------------------------------------------#
  # Expand the grp_history object to an ind_history object.
  #-------------------------------------------------------------#
  # ind_history        <- vector("list", dat$n_subj)
  # names(ind_history) <- dat$file_order$names
  # ind_history        <- lapply(seq_along(ind_history), function(i){
  #   ind_history[[i]] <- grp_history
  # })
  # names(ind_history) <- dat$file_order$names
  ...
  if (subgroup){
    ...
    #-------------------------------------------------------------#
    subgrp_history  <- list(); cnt<-1
    
    for(i in 1:length(grp)){
      
      sub_pruned_grp_paths <- setdiff(grp[[i]]$group_paths,grp_sub[[i]]$group_paths)
      
      sub_pruned_grp_paths <- unique(grp_history[[length(grp_history)]][[i]]$pruned, sub_pruned_grp_paths)
      
      grp_level_paths      <- grp_history[[length(grp_history)]][[i]]$add_syntax
      
      if(length(sub_pruned_grp_paths) == 0){
        
        sub_pruned_grp_paths <- NA
        
      } 
      
      for(j in 1:sub[[i]]$n_subgroups){
        
        sol_i_subgrp_j_hist            <- grp_history[[length(grp_history)]][[i]]
        sol_i_subgrp_j_hist$add_syntax <- c(grp_level_paths, sub_spec[[i]][[j]]$sub_paths)
        sol_i_subgrp_j_hist$n_paths    <- sub_spec[[i]][[j]]$n_sub_paths
        sol_i_subgrp_j_hist$pruned     <- sub_pruned_grp_paths
        sol_i_subgrp_j_hist$stage      <- "subgroup"
        sol_i_subgrp_j_hist$grp_sol    <- i
        sol_i_subgrp_j_hist$subgrp_sol <- j
        
        subgrp_history[[cnt]] <- sol_i_subgrp_j_hist
        cnt <- cnt + 1
    ...
    history <- append(grp_history, grp_history[length(grp_history)])
    history[[length(history)]] <- subgrp_history
  ...
  if(!ms_allow){
    ...
    # old_ind <- ind
    # old_sub <- sub
    # old_sub_spec <- sub_spec
    # 
    # #-----------------------------------------------#
    # # Individual level search                       #
    # #-----------------------------------------------#
    # ind_history <- lapply(seq_along(grp), function(j){
    #   
    #   if(ms_allow){
    ...
    #   
    #   if(subgroup){
    #     indiv.search.ms(dat, grp[[j]], sub_all[[j]]$ind, ms_tol, ms_allow)
    #   } else {
    #     indiv.search.ms(dat, grp[[j]], ind, ms_tol, ms_allow)
    #   }
    #   
    # })
