...
#' @keywords internal 
prune.paths2 <- function(base_syntax, 
                        fixed_syntax,
                        add_syntax,
                        data_list, 
                        n_paths, 
  ...
  stage <- ifelse(subgroup_stage, "subgroup prune", "group prune")
  
  history <- append(history, history[[length(history)]])
  history[[length(history)]]$ stage <- stage
