...
#'  \itemize{
#'  \item{allPathSummaryCounts} {Contains summary count information for paths identified at the group-, subgroup (if subgroup = TRUE), and individual-level.}
#'  \item{allPathEstimates} {Contains estimate, standard error, p-value, and z-value for each path for each individual}
#'  \item{summaryFit} {Contains model fit information for individual-level models. If subgroups are requested, this file also contains the subgroup membership for each individual.}
#'  \item{finalPathCounts} Contains counts of total number of paths, both contemporaneous and lagged, estimated for the sample. The row variable is the outcome and the column variable is the predictor variable. .
#'  }
#'  In subgroup output directory (if subgroup = TRUE):
#'  \itemize{
#'  \item{subgroupkPathCounts} Contains counts of relations among lagged and contemporaneous variables for the kth subgroup.
#'  \item{subgroupkPlot} Contains plot of group, subgroup, and individual level paths for the kth subgroup. Black represents group-level paths, grey represents individual-level paths, and green represents subgroup-level paths.
#'  }
#'  Note: if a subgroup of size n = 1 is discovered,subgroup-level output is not produced.
#'  In individual output directory:
#'  \itemize{
#'  \item{idBetas} Contains individual-level estimates of each path for each individual.
#'  \item{idStdErrors} Contains individual-level standard errors for each path for each individual.
#'  \item{idPlot} Contains individual-level plots. Red paths represent positive weights and blue paths represent negative weights.
#' }
...
#                 col.names = FALSE)
    ...
    if (plot==TRUE){
      ...
      if (agg==TRUE) plotind <- file.path(out,"aggregatePathsPlot.pdf")
      pdf(plotind)
